...
import { API_WS_HOST } from '/host.js';
import { USE_WEBSOCKET } from '/host.js';
...
recordBtn.addEventListener("click", async () => {
    if (!recording) {
        ...
        try {
            ...
            if (USE_WEBSOCKET)
                await startChatWithWebsocket(chatHistory);
            else
...
async function startChatWithWebsocket(messages) {
    const audioContext = new AudioContext();
    let fullResponse = "";
    ...
    try {
        ...
        const ws = new WebSocket(`${API_WS_HOST}/ws`); // Use WebSocket URL

        ws.onopen = () => {
            console.log("WebSocket connected");

            // Send messages as JSON through WebSocket
        ...
        ws.onmessage = async (event) => {
            ...
            // Handle session ID (only sent once if generated by server)
            if (data.session_id) {
            ...
            if (data.status === "STREAM") {
                if (data.text_chunk) {
                    fullResponse += data.text_chunk;
                    addMessage("assistant", data.text_chunk); // Display text message
                    console.log("Received text:", data.text_chunk);
                } else {
                    console.error("Received no text from message");
                }

                if (data.audio_chunk) {
                    enqueueAudioChunk(data.audio_chunk)
                }
            } else if (data.status === "FINISHED") {
                chatHistory.push({ role: "assistant", content: fullResponse });
                fullResponse = "";
                ws.close(); // Close WebSocket when streaming is done
            }
        };

        ws.onerror = (error) => {
            console.error("WebSocket error:", error);
            displayErrorMessage("WebSocket connection error.");
        };
...
async function processQueue() {
    while (audioQueue.length > 0 && !isPlaying) {
        ...
        try {
            await playAudioChunk(base64Audio, globalAudioContext);
        } catch (error) {
...
// Function to enqueue a new audio chunk
